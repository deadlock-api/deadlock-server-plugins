...
namespace Deathmatch.Stats;

// PostHog /capture/ client. Fire-and-forget: never await from a game thread.
// Mirrors TrooperInvasion/Stats/StatsClient.cs — one shared HttpClient, single
// 10s timeout, catch-and-log, no retries. Silent no-op if either
// DEADWORKS_ENV_POSTHOG_KEY or DEADWORKS_ENV_STATS_SALT is unset, so the
...
internal static class StatsClient
{
    ...
    {
        // Best-effort: any bootstrap failure leaves _enabled=false so the
        // gamemode keeps running. Never throw out of Configure — the plugin's
        // OnLoad would propagate it.
        try
        {
            ...
            _gameMode = Env("GAME_MODE") ?? "deathmatch";
            _region = Env("REGION") ?? "";

            if (string.IsNullOrWhiteSpace(key) || string.IsNullOrWhiteSpace(salt))
            {
                _enabled = false;
                Console.WriteLine("[DM.Stats] Disabled: POSTHOG_KEY or STATS_SALT missing.");
                return;
            }
            ...
            _enabled = true;
            Console.WriteLine($"[DM.Stats] Enabled: host={_host} server={_serverId} mode={_gameMode} region={_region}");
        }
        catch (Exception ex)
        {
            _enabled = false;
            Console.WriteLine($"[DM.Stats] Disabled: Configure failed: {ex.Message}");
        }
    }
    ...
    public static string? HashSteamId(ulong steamId)
    {
        ...
        {
            Console.WriteLine($"[DM.Stats] HashSteamId failed: {ex.Message}");
            return null;
        }
    }
    ...
    public static void Capture(string eventName, string? distinctId = null, IDictionary<string, object?>? props = null)
    {
        ...
        {
            Console.WriteLine($"[DM.Stats] capture '{eventName}' dispatch failed: {ex.Message}");
        }
    }
    ...
    private static async Task SendAsync(string eventName, string distinctId, IDictionary<string, object?>? props)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
                Console.WriteLine($"[DM.Stats] capture '{eventName}' HTTP {(int)response.StatusCode}");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[DM.Stats] capture '{eventName}' failed: {ex.Message}");
        }
    }
